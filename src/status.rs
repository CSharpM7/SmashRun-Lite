...
pub unsafe fn fighter_pickup_light(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    ) 
    {
        ...
        if vars::SPIRIT_TYPE[entry] == 100 {
            vars::SPIRIT_TYPE[entry] = app::sv_math::rand(hash40("fighter"), 37);
        }
        let is_All = vars::SPIRIT_TYPE[entry] == 101;
        let is_Attack = [0,1,2,3,4,5,6].contains(&vars::SPIRIT_TYPE[entry]) && vars::LEVEL_ATTACK[entry] < vars::LEVEL_MAX;
        let is_Def = [7,8,9,10,11,12,13].contains(&vars::SPIRIT_TYPE[entry]) && vars::LEVEL_DEF[entry] < vars::LEVEL_MAX;
        let is_Speed = [14,15,16,17,18,19,20].contains(&vars::SPIRIT_TYPE[entry]) && vars::LEVEL_SPEED[entry] < vars::LEVEL_MAX;
        let is_Jump = [21,22,23,24,25,26,27].contains(&vars::SPIRIT_TYPE[entry]) && vars::LEVEL_JUMP[entry] < vars::LEVEL_MAX;
        let is_Turbo = vars::SPIRIT_TYPE[entry] >= 28 && vars::SPIRIT_TYPE[entry] < 100 && vars::LEVEL_TURBO[entry] < vars::LEVEL_MAX;
        ...
        if is_Attack || is_All
        {
            ...
            println!("Food type: {}, Attack level: {}",vars::SPIRIT_TYPE[entry],vars::LEVEL_ATTACK[entry]);
        }
        //Def
        if is_Def || is_All
        {
            ...
            println!("Food type: {}, Def level: {}",vars::SPIRIT_TYPE[entry],vars::LEVEL_DEF[entry]);
        }
        //Speed
        if is_Speed || is_All
        {
            ...
            println!("Food type: {}, Speed level: {}",vars::SPIRIT_TYPE[entry],vars::LEVEL_SPEED[entry]);
        }
        //Jump
        if is_Jump || is_All
        {
            ...
            println!("Food type: {}, Jump level: {}",vars::SPIRIT_TYPE[entry],vars::LEVEL_JUMP[entry]);
        }
        //Turbo
        if is_Turbo || is_All
        {
            ...
            println!("Food type: {}, Turbo level: {}",vars::SPIRIT_TYPE[entry],vars::LEVEL_TURBO[entry]);
        }
        if is_All
        {
...
pub unsafe fn fighter_dead(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    vars::LEVEL_ATTACK[entry] = 0.max(vars::LEVEL_ATTACK[entry]-5);
    vars::LEVEL_SPEED[entry] = 0.max(vars::LEVEL_SPEED[entry]-5);
    vars::LEVEL_DEF[entry] = 0.max(vars::LEVEL_DEF[entry]-5);
    vars::LEVEL_JUMP[entry] = 0.max(vars::LEVEL_JUMP[entry]-5);
    vars::LEVEL_TURBO[entry] = 0.max(vars::LEVEL_TURBO[entry]-5);
